...
class Net(nn.Module):
    def __init__(self):
      ...
      self.model = nn.Sequential(
        nn.Linear(1048576, 262144),
        nn.ReLU(),
        nn.Linear(262144, 65536),
        nn.ReLU(),
        nn.Dropout(0.2),
...
vectorizer = Model('vect_wortschartz_30_v5').model
...
def train_loop(dataloader, model, loss_fn, optimizer, device):
    ...
    with tqdm(total=size) as pbar:
        for batch, data in enumerate(dataloader):
            X = torch.tensor(vectorizer.transform(data['text'])).type(torch.float32).to(device)
